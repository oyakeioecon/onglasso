...
for it in trange(iterations):        
    for i in range(p):            
        ...
        ind_e = ind_exclude[:, i]
        s_22 = S[i, i]
        omega_12 = Omega[ind_e, i]
        inv_Omega_11 = sl.inv(exclude_rowcol(Omega, ind_e, ind_e))
        # Step 2: sample omega_22
        Omega[i, i] = st.gamma.rvs(a = shape_gam, loc = quad_form(inv_Omega_11, omega_12), scale = 1.0 / (s_22/2 + lambda_22))
        # Step 3: Hit-and-Run algorithm for sampling omega_12
        # We used some while roops to avoid errors 
        c = quad_form(inv_Omega_11, omega_12) - Omega[i, i]    
        while True:    
            z = rnd.randn(p - 1)
            alp = z / sl.norm(z)
            s_12 = -S[ind_e, i]
            b = omega_12 @ inv_Omega_11 @ alp
            a = quad_form(inv_Omega_11, alp)
            inv_C = (s_22 + 2*lambda_22) * inv_Omega_11 + np.diag(1.0 / (Tau[ind_e, i]))
            var_kappa = 1.0 / quad_form(inv_C, alp)
            if var_kappa > 0:
                break  
        mu_kappa = var_kappa * (alp @ (s_12 - inv_C @ omega_12))
        ...
        omega_12 += kappa * alp
        Omega = update_offdiagonals(Omega, omega_12, ind_e, i)             
        if sl.det(Omega) > 0:
            hrs_det[i,it] = 1.0
        else: 
            hrs_det[i,it] = 0.0
        # Step 4: sample lambda_12
        spost = s*np.ones((p-1,)) + np.abs(omega_12)
        lambda_12 = rnd.gamma(rpost, 1/spost) 
        # Step 5: sample tau_12
        tau_12 = 1 / rand_ig(np.sqrt(lambda_12**2/omega_12**2),lambda_12**2,p-1)
