...
while True:
    if num == str(1) or num == str(2) or num == str(3) or num == str(4) or num == str(5) or num == str(6):
        ...
        num = input()
print('Thank you. We confirmed all your inputs:n=',n,'p=',p, model,'model. Then we will compute Hit-and-Run sampler and output results. Please wait for the computation to finish.')
...
for it in trange(iterations):        
    for i in range(p):            
        ...
        # This if statement exists to avoid the calculation not being able to
        # start when the initial value of c is >= 0. 
        # Once sampling is started with c < 0, c < 0 is retained during the sampling.
        if it == 0:
            if c >= 0:
                c = -1
